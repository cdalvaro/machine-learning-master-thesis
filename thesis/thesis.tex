...
% TODO: Fix table if figures changes
\begin{table}[h!]
  \begin{center}
...
The last step presents a table with candidates objects and allows to export this dataset to TOPCAT, Aladin and VOSA to perform a deeper
filtering and analysis based on the results given by Clusterix.
...
As we can see in Figure \ref{fig:topcat_prob_selection_melotte_22}, the selection is too restrictive and we have lost too much stars.
...
For the sake of completeness, Table \ref{tab:results_melotte_22} is shown again, in order to compare the results obtained with the validation method and our method.
...
Until now, we have focused our study in Melotte 22 OC. Since our main aim is to get a model that fits well for a wide range of clusters,
