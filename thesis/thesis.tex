...
To achieve the general goal, we'll follow the following milestones:
...
As mentioned before, we want to characterize clusters looking at their dynamics properties.
...
information for approximately 1,600 million stars, but it is still being too large.
...
(with a radius multiplier to increase the area to be downloaded) and save the downloaded data inside the custom database hosted in the second container.
...
file which automatically launches one instance of each container.
