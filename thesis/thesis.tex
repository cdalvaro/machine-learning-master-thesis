...
So, as summary, these are the features we are going to use as sources for our clustering models:

\begin{itemize}
  \item \emph{Proper motion in right ascension}: $\mu_{\alpha}$
  \item \emph{Proper motion in declination}: $\mu_{\delta}$
...
The final selection is the one with best silhouette score. This process is explained in more detail in Appendix \ref{chap:feature_selection}.

\section{Soft Clustering with K-Means}

Once we have found the set of features that best describes our problem, we can begin to search for the OC in the study region.
...
one small cluster and another with the rest of stars. This effect complicates our task of finding the desired OC. Therefore, we have to find
a way to set the right value for the number of clusters to isolate the searched cluster without creating to many groups.
